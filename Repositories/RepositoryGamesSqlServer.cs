...
namespace CozyGames.Repositories
{
    public class RepositoryGamesSqlServer
    {
        ...
        public async Task DeleteRating(int idRating)
        {
            var ratingToDelete = await this.context.LikeComentarios
                                          .FirstOrDefaultAsync(z => z.IdRating == idRating);
            

            if (ratingToDelete != null)
            {
                this.context.LikeComentarios.Remove(ratingToDelete);
                await this.context.SaveChangesAsync();
            }
        }
        ...
        public async Task UpdateLikeComentarioAsync(int commentid, int idusuario)
        {
            LikeComentario like = await this.context.LikeComentarios.FirstOrDefaultAsync(l => l.IdRating == commentid && l.IdUsuario == idusuario);
            if (like == null)
            {
                await LikeRating(idusuario, commentid);
            }
            else
            {
